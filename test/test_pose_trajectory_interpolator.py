...
from common.pose_trajectory_interpolator import (
    rotation_distance,
    pose_distance,
...
def test_rotation_distance():
    def rotation_distance_align(a: st.Rotation, b: st.Rotation) -> float:
        return st.Rotation.align_vectors(b.as_matrix().T, a.as_matrix().T)[
            0
        ].magnitude()
...
def test_pose_trajectory_interpolator():
    ...
    interp = PoseTrajectoryInterpolator([0, 1, 3], np.random.randn(3, 7))
    times = interp.times
    poses = interp.poses
...
def test_add_waypoint():
    ...
    for i in tqdm(range(10000)):
        ...
        interp = PoseTrajectoryInterpolator(times=waypoint_times, poses=waypoint_poses)
...
def test_drive_to_waypoint():
    ...
    for i in tqdm(range(10000)):
        rng = np.random.default_rng(i)
        n_waypoints = rng.integers(1, 5)
        waypoint_times = np.sort(rng.uniform(0, 1, size=1))
        insert_time = rng.uniform(-0.1, 1.1)
        curr_time = rng.uniform(-0.1, 1.1)
        max_pos_speed = rng.poisson(3) + 1e-3
        max_rot_speed = rng.poisson(3) + 1e-3
        waypoint_poses = rng.normal(0, 3, size=(1, 7))
        new_pose = rng.normal(0, 3, size=7)
