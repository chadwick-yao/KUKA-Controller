    ...
    if clientID != -1:
        ...
        while time.time() - startTime < 0.05:
            returnCode, data = sim.simxGetIntegerParameter(
                clientID, sim.sim_intparam_mouse_x, sim.simx_opmode_buffer
            )  # Try to retrieve the streamed data
            if (
                returnCode == sim.simx_return_ok
                ...
                )  # Mouse position x is actualized when the cursor is over CoppeliaSim's window
            time.sleep(0.005)
