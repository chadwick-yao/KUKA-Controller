...
from codebase.shared_memory.shared_memory_ring_buffer import (
...
def test():
    ...
    ring_buffer = SharedMemoryRingBuffer.create_from_examples(
        shm_manager, {"timestamp": np.array(0, dtype=np.float64)}, buffer_size=128
    )
    for i in range(30):
        ring_buffer.put({"timestamp": np.array(time.perf_counter(), dtype=np.float64)})
...
def _timestamp_worker(ring_buffer, start_event, stop_event):
    while not stop_event.is_set():
        start_event.set()
        ring_buffer.put({"timestamp": np.array(time.time(), dtype=np.float64)})
...
def test_mp():
    ...
    stop_event = mp.Event()
    worker = mp.Process(
        target=_timestamp_worker, args=(ring_buffer, start_event, stop_event)
    )
    worker.start()
    start_event.wait()
    for i in range(1000):
...
def test_get_last_k():
    ...
    shm_manager.start()
    ring_buffer = SharedMemoryRingBuffer.create_from_examples(
        shm_manager, {"counter": np.array(0, dtype=np.int64)}, buffer_size=8
    )
    ...
    for i in range(100):
        ring_buffer.put({"counter": np.array(i, dtype=np.int64)})
        last_k.append(i)
        if i > k:
...
def test_timing():
    ...
    for i in range(1000):
        ...
        ring_buffer.put({"counter": np.array(i, dtype=np.int64)}, wait=False)
        if (i % 10 == 0) and (i > 0):
...
def _timestamp_image_worker(ring_buffer, img_shape, dt, start_event, stop_event):
    ...
    while not stop_event.is_set():
        t_start = time.monotonic()
        start_event.set()
        ring_buffer.put({"img": image, "timestamp": time.time(), "counter": i})
        t_end = time.monotonic()
        desired_t = (i + 1) * dt + t_init
        # print('alive')
        if desired_t > t_end:
...
def test_timing_mp():
    ...
    stop_event = mp.Event()
    worker = mp.Process(
        target=_timestamp_image_worker,
        args=(ring_buffer, img_shape, 1 / hz, start_event, stop_event),
    )
    worker.start()
    start_event.wait()
    out = None
