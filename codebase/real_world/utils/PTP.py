...
## Arc motions
    ...
    def movePTPArcXY_AC(self,theta,c,vel):
        assert len(theta) == 1, 'Error in function [movePTPArcXY_AC]: Rotation angle should be a scalar'
        assert len(c) == 2, 'Error in function [movePTPArcXY_AC]: Center of rotation should be an array of two elements [x, y]'
        assert len(vel) == 1, 'Error in function [movePTPArcXY_AC]: Velocity should be a scalar'
        k = [0, 0, 1]
        pos = self.getter.get_EEF_pos()
        c1 = [c[0], c[1], pos[2]]
    ...
    def movePTPArcXZ_AC(self, theta, c, vel):
        assert len(theta) == 1, 'Error in function [movePTPArcXZ_AC]: Rotation angle should be a scalar'
        assert len(c) == 2, 'Error in function [movePTPArcXZ_AC]: Center of rotation should be an array of two elements [x, z]'
        assert len(vel) == 1, 'Error in function [movePTPArcXZ_AC]: Velocity should be a scalar'
    ...
    def movePTPArcYZ_AC(self,theta,c,vel):
        assert len(theta) == 1, 'Error in function [movePTPArcYZ_AC]: Rotation angle should be a scalar'
        assert len(c) == 2, 'Error in function [movePTPArcYZ_AC]: Center of rotation should be an array of two elements [x, z]'
        assert len(vel) == 1, 'Error in function [movePTPArcYZ_AC]: Velocity should be a scalar'
    ...
    def movePTPCirc1OrintationInter(self, f1,f2, relVel):
        assert len(f1) == 6, 'Error in function [movePTPCirc1OrintationInter]: The first frame should be an array of 6 elements [x, y, z, alpha, beta, gamma]'
        assert len(f2) == 6, 'Error in function [movePTPCirc1OrintationInter]: The second frame should be an array of 6 elements [x, y, z, alpha, beta, gamma]'
        assert len(relVel) == 1, 'Error in function [movePTPCirc1OrintationInter]: Relative velocity should be a scalar'
    ...
    def movePTPLineEEF(self, pos, vel):
        assert len(vel) == 1, 'Error in function [movePTPLineEEF]: Velocity shall be a scalar'
        assert len(pos) == 6, 'Error in function [movePTPLineEEF]: Position should be an array of 6 elements'
    ...
    def movePTPLineEefRelEef(self, pos, vel):
        assert len(vel) == 1, 'Error in function [movePTPLineEefRelEef]: Velocity should be a scalar'
        assert len(pos) == 3, 'Error in function [movePTPLineEefRelEef]: Position should be an array of 3 elements [x, y, z]'
...
# joint space
    def movePTPJointSpace(self, jpos, relVel):
        assert len(jpos) == 7, 'Error in function [movePTPJointSpace]: Joints positions shall be an array of 7 elements'
        assert len(relVel) == 1, 'Error in function [movePTPJointSpace]: Relative velocity should be a scalar'
    ...
    def movePTPHomeJointSpace(self, relVel):
        assert len(relVel) == 1, 'Error in function [movePTPHomeJointSpace]: Relative velocity should be a scalar'
    ...
    def movePTPTransportPositionJointSpace(self, relVel):
        assert len(relVel) == 1, 'Error in function [movePTPTransportPositionJointSpace]: Relative velocity should be a scalar'
        jpos = [0, 0, 0, 0, 0, 0, 0]
