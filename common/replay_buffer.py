...
class ReplayBuffer:
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]) -> None:
        """
        Dummy constructor. Use copy_from and create_from classmetrod instead.
        """
        assert "data" in root
        assert "meta" in root
        assert "episode_ends" in root["meta"]

        # make sure every single data group has the same length
    ...
    def create_empty_zarr(cls, storage=None, root=None):
        if root is None:
            if storage is None:
                storage = zarr.MemoryStore()  # default -> memory store
    ...
    def create_from_group(cls, group, **kwargs):
        if "data" not in group:
            buffer = cls.create_empty_zarr(
                root=group, **kwargs
            )  # recreate from scratch
        else:
            buffer = cls(root=group, **kwargs)  # already exist
        return buffer
